...
- `cas-*.cls` and `cas-common.sty`: Elsevier CAS template assets.
- `IEEEtran.bst`: the bibliography style in use (IEEE numbered). `cas-model2-names.bst` ships with the template for author--year journals and is currently unused.
- `PLACEHOLDERS.md`: every unresolved placeholder in `main.tex` and where its value comes from.
