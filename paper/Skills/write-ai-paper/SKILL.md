...
Unless the user or the target journal specifies otherwise:

- Citation style is **IEEE numbered**: `\usepackage[numbers,sort&compress]{natbib}`
  with `\bibliographystyle{IEEEtran}`, and `\cite{key}` everywhere. Never use
  `\citep` or `\citet`. See [Introduction.md](Introduction.md), section 43.
- The manuscript class is `cas-sc` (Elsevier CAS, single column); switch to
  `cas-dc` only when the journal requires a double-column layout.
- The manuscript stays in one file, `main.tex`.
