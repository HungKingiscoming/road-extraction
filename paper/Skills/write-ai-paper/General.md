...
in manuscript prose, and do not write its LaTeX source form `---`.
...
# 6. Single Hyphen Only

**Never emit `--` or `---` in the LaTeX source.** Write a single hyphen `-`.

This is a hard formatting rule for this project. In LaTeX, `--` compiles to an en
dash and `---` to an em dash, so both must be avoided in generated text.
...
`ResNet-34 -- the shared encoder -- returns five levels`
...
The rule applies to every hyphen-like use: numerical ranges, paired concepts,
compound relations, section titles, table cells, figure captions, and author
affiliations. A single `-` is the only permitted form.

Where a dash was being used as sentence punctuation, do not substitute a hyphen.
Rewrite with a comma, semicolon, colon, parentheses, or a separate sentence, as
section 5 requires.

**One exception:** the `pages` field of a BibTeX entry keeps the standard
`pages = {749--753}` form, because the bibliography style, not the manuscript,
renders it. The rule above governs `main.tex`; it does not govern
`references.bib`.

Before delivering, grep `main.tex` for `--` and confirm there are no matches.
