...
The default citation style for this project is **IEEE numbered**: references are
cited as bracketed numbers in order of first appearance, and the reference list
is ordered by that number rather than alphabetically.

Preamble and bibliography setup:
...
The `sort&compress` option renders consecutive citations as a range, so
`\cite{a,b,c}` becomes `[1]-[3]` rather than `[1], [2], [3]`.

Use `\cite{key}` for every citation. Do **not** use `\citep` or `\citet`: both
are author-year commands, and under a numbered `.bst` they either degrade to a
bare number or produce inconsistent output.

Use the bracketed number as a supporting reference, not as a noun:

```latex
... remains difficult under limited annotated data \cite{smith2025method,jones2024review}.
```

When the authors are the grammatical subject, write the names explicitly and
attach the number:

```latex
Smith et al.~\cite{smith2025method} proposed ...
```

Never write `\cite{smith2025method} proposed ...`, which reads as "[7] proposed".
A citation number is not a noun phrase and a sentence must remain grammatical if
every bracketed number is deleted.

Use a non-breaking space before the citation when it follows a name
(`Smith et al.~\cite{...}`).

Never create a citation key unless it corresponds to a verified real bibliographic entry.

If a target journal requires author-year instead, switch the whole manuscript at
once: change the `natbib` options, change the `.bst`, and convert every `\cite`
to `\citep` or `\citet` as the sentence requires. Do not mix the two styles.
